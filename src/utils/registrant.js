...
export const assignStationName = reg => {
	const StationName =
		window.localStorage.getItem("validar_stationName") || "-NO STATION NAME-";
...
export const assignAsAttended = reg => {
	const FirstCheckInDateTime = reg.FirstCheckInDateTime
		? reg.FirstCheckInDateTime
		: new Date();
	return Object.assign({}, reg, {
		Attended: true,
...
export const generatePrintArgs = (reg, printerName, coworking) => {
	const printArgs = {
	...
	printArgs.printDocument = generatePrintDoc(reg, coworking);

	return printArgs;
};

// Generate printdoc to send
const generatePrintDoc = (reg, coworking) => {
	// Print Doc
	const printDocString = "<print><printitems></printitems></print>";
	const printParser = new DOMParser();
	const printDoc = printParser.parseFromString(
		printDocString,
		"application/xml"
	);
	...
	// Survey Data Doc
	const surveyData = reg.SurveyData
		? reg.SurveyData
		: "<result><responses></responses></result>";
	...
	appendXmlPrintItem(printDoc, "slot5", coworking ? "CW" : "EV");
