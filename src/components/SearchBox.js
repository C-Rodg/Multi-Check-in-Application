...
class SearchBox extends Component {
	constructor(props) {
		...
		if (this.props.location && this.props.location.state) {
			const settingsStr = window.localStorage.getItem(
				this.props.location.state.event.campaign
			);
			this.state = {
	...
	onSearchSubmit(val) {
		...
		axios
			.post("Services/Methods.asmx/SearchRegistrants", inputArgs)
			.then(resp => {
				const { d } = resp.data;
				if (d && !d.Fault) {
					...
					if (
						this.state.selectedEvent.coworking ||
						this.state.settings.walkin
					) {
						filteredList = masterList;
					} else {
						const currentEventName = this.state.selectedEvent.name
							.replace(/\W/g, "")
							.toUpperCase();
						filteredList = masterList.filter(person => {
							...
							let allowedEvent = getTextFromXml(
								responsesElement,
								"qrEventName"
							);
							if (allowedEvent) {
					...
					if (filteredList && filteredList.length === 1) {
						...
						if (this.state.settings.walkin) {
							message.warn(
								"No registration found. Please complete the form below to continue checking in.",
								3
							);
							this.props.history.push({
								pathname: "/walkin",
							...
							message.warn(
								"Walk-ins are not allowed for this event. Please see the help desk for more information..",
								3
							);
						}
					} else {
	...
	render() {
		return (
			<div
				...
				style={
					this.state &&
					this.state.registrants &&
					this.state.registrants.length > 8
						? searchBoxStyles.wrapperAbs
			...
			>
				<Row
					...
					align="middle"
					style={searchBoxStyles.searchRow}
				>
					<Col span={12}>
				...
				{this.state &&
				this.state.registrants &&
				this.state.registrants.length > 1
