...
class RegistrantBox extends Component {
	...
	componentWillMount() {
		if (
			!this.state.event.campaign ||
			!this.state.masterRecord.hasOwnProperty("AttendeeGuid")
		) {
			message.error(
				"Something appears to be very wrong. Please go back and try again...",
				5
			);
		}
	...
	onFormSubmit(ev) {
		...
		const newRegistrant = assignRegistrantProps(
			assignAsAttended(
				assignStationName(copyMasterRecord(this.state.masterRecord))
			),
			form
		);
		newRegistrant.SurveyData = convertFormToSurveyData(
			form,
			newRegistrant.SurveyData
		);
								...
								<Row>
									<Col span={11}>
										<Row style={registrantBoxStyles.row}>
											<Col
												span={10}
												style={registrantBoxStyles.label}
												className="req"
											>
												First Name:
											</Col>
										...
										{!this.state.event.coworking && this.state.settings.prereg
											? <Row style={registrantBoxStyles.row}>
													<Col
														span={17}
														style={registrantBoxStyles.optInLabel}
														className="req"
													>
									...
									<Col span={11} offset={2}>
										...
										{!this.state.showStateOther
											...
											: <Row style={registrantBoxStyles.row}>
													<Col span={10} style={registrantBoxStyles.label}>
														State Other:
													</Col>
													<Col span={14}>
														<Input
										...
										<Row style={registrantBoxStyles.row}>
											<Col
												span={10}
												style={registrantBoxStyles.longLabel}
												className="req"
											>
												Level of AWS Usage:
											</Col>
